...
const sheetVariants = cva(
  "fixed z-50 bg-black border-white/10 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      <SheetPrimitive.Close className="absolute right-5 top-5 rounded-sm text-white/60 hover:text-white transition-colors focus:outline-none">
        <X className="h-5 w-5" />
